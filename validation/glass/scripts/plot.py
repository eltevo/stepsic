#!/usr/bin/env python3
'''
Plot 2D slices of glass particle distributions for the stepsic / StePS paper.

Produces a 4-panel landscape figure (A&A double-column width)
showing X-Z slices through each glass geometry:
...
The slab is centred on the median Y coordinate and its half-width is
controlled by ``--slice-thickness``. For the cubical grid panel the
script selects the single Y-layer closest to the centre, since a
finite-thickness slab would either show one layer or mix two.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description='Plot 4-panel 2D glass validation figure (X-Z slices) '
