...
def test_resolution_tag_matches_string_spec(params, expected):
    """expected tags are literal cases from the naming specification."""
    assert _resolution_tag(params) == expected


def test_run_dirname_matches_string_spec(tiny_params):
    """expected name is assembled independently from tiny-config values."""
...
def test_ensure_run_dir_is_scoped_and_idempotent(tmp_path, tiny_params):
    """repeated creation returns the same directory beneath IC_DIR."""
    ...
    assert first.is_dir()
    # the hostile-looking prefix is a literal path component, not shell input.
